...
- **Skill 不跨站、不登入** — **瀏覽器由 AI 啟動**（playwright MCP 開空白頁；持久 profile 常保留既有登入 session）；**視窗滿版／放哪台螢幕、導航到站點、登入，一律由使用者自己做**，並**建議停在該品牌遊戲大廳**（另開好後台投注報表分頁），Skill 從當前頁接手；**品牌內選款、進入/退出遊戲由 AI 自行操作**（calibrate 自挑大廳第一款當 sample、run 逐款自開）。**同站內品牌切換 AI 可代勞**：當前分頁已是目標站點、只是停在別的品牌分類時，AI 先宣告再切到目標品牌；**不在目標站點／未登入／多站分不清目標**才停下請使用者處理。跨站導航與登入永不代勞。
