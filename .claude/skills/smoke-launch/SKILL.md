---
name: smoke-launch
description: 跨品牌「遊戲載入冒煙檢查」—— 逐款點開，確認能不能載入到 start/splash 畫面，產一份標註哪些能載入的 md。不下注、不驗餘額，故不產生任何 PASS。適用於新測試環境盤點「哪些品牌/遊戲已開通」。當使用者說「確認遊戲能不能開」「載入測試」「盤點哪些遊戲可以玩」時用。站點隱含於當前頁面；`.env` 白名單內的站可自行導航與登入，白名單外不代勞。
---
...
## 起手（所有 mode 共用）
瀏覽器由 AI 啟動（`browser_navigate` 到 `about:blank`）。AI 從當前頁接手，同站內切分類/品牌可代勞（先宣告）。
**視窗尺寸**：有人在場＝使用者手動調；無人值守＝`playwright-mcp.unattended.json` 的 `contextOptions.viewport` 釘死。
**導航與登入**：站點在 `.env` 白名單內（`SITEn_HOST`）且有 `SITEn_USER`/`SITEn_PASS` → AI 可自行導航並代填（先宣告，登入後仍要實際驗登入態）；
🔴 **不在白名單內一律停下請使用者處理，不代勞導航、不代填帳密**，也不要從歷史紀錄/書籤猜站點。憑證只進輸入框，不進對話/報告/截圖。
詳見 CLAUDE.md「核心不變量」的登入例外條款。
...
🔴 **五個前提缺一不可（1–3 在同一天各踩過一次，4–5 是 2026-08-14 踩到的）：**
...
4. 🔴 **body 要攔真實請求來抄，不能照欄位名猜值** —— 抄 header 還不夠。實測某站的 launch body 有個 `haveLobby` 布林欄，我依遊戲清單的欄位自組時填了 `false`，結果**全站 25 個平台一律回 `400 ThirdPartyError`，包括三天前確認可載入的那幾個**。改成 `true` 後可載入的平台立刻回 200 帶真實 gameUrl。**一個布林欄就能讓整站假陰性，而且錯誤訊息是合理的業務錯誤（「開啟遊戲失敗，請稍後再嘗試」），毫無徵兆。**
   攔法：進頁面後 monkey-patch `window.fetch` 與 `XMLHttpRequest.prototype.open/send` 記下符合端點的 url/body，然後**用 UI 真的點一次**再讀下來。
5. 🔴 **一定要跑對照組才可信** —— 預篩方法上線前，至少各測一個**已知會成功**與**已知會失敗**的樣本（例如上一輪確認可載入／確認掛掉的品牌），兩者都符合預期才採用。沒有對照組就無法區分「站方全掛」與「我的請求寫錯」。最好再加一個 UI 交叉印證（實測某品牌 API 失敗、UI 點下去也確實開不出，才敢下結論）。

另外：**自組 API 請求前，先從 network 面板抄該站真實送出的 header**（`authorization`／`companycode`／`lang`／**`domain`**）。`domain` 常與頁面網址不同（實測過某站的 `domain` 比頁面網址多了一段後綴），填錯**不會報錯，而是安靜回一組看起來合理但全錯的資料**。（但也實測過另一站 `domain` 填錯對回應完全無影響 —— 所以 header 與 body 都以**攔到的真實值**為準，不要靠推論。）
...
- **導航與登入**：`.env` 白名單內的站可代勞（先宣告）；**白名單外不代勞導航、不代填帳密**。憑證只進輸入框。
