...
- 🔴 **確認該品牌遊戲錢包已有餘額**（不只是登入）：很多第三方電子品牌有**獨立遊戲錢包**，要先從主錢包轉帳進去才能下注。沒轉錢的症狀＝遊戲內餘額 0／下注點了不登錄／後台 0 筆。跑前提醒使用者「已轉帳到該品牌錢包」，餘額為 0 就停下請使用者先儲值，不要硬跑（會整批 BET_NOT_PLACED）。
...
- 使用者確認已開好、已篩好，才往下做。
- 提醒：第三方注單**回報後台可能延遲數分鐘**，剛跑完就對帳可能還沒進，`backoffice-reconciler` 會 poll 等候。
