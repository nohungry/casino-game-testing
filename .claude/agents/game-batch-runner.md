...
遮罩／彈窗攔截 pointer events 的症狀與「手勢不對」完全一樣：點了沒反應、甚至連 API 都不發，看起來像網站壞掉。三個 stage 站都遇過，其中一次 agent 誤診成「canvas 手勢無效」並花時間升級手勢，實際上是彈窗又出現吃掉點擊 —— **它的裁切截圖看不到，只有全頁截圖才發現**。
（這是 `feedback_screenshot_cost_context_bloat` 省截圖原則的**明文例外**：常態量測用裁切，點擊無效時例外照全頁，看完立刻丟棄不留存。）
